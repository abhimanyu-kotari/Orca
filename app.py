...
def _generate_route_pdf(
    ...
    """Generate a compact PDF route report for download."""
    ...
    document = SimpleDocTemplate(
        ...
        bottomMargin=14 * mm,
        title=route_title,
        author="ORCA Marine Intelligence",
    )
    styles = getSampleStyleSheet()
    title_style = ParagraphStyle(
        "RouteTitle", parent=styles["Title"], alignment=TA_CENTER,
        textColor=colors.HexColor("#0B3954"), spaceAfter=8,
    )
    body_style = ParagraphStyle("RouteBody", parent=styles["BodyText"], fontSize=9, leading=12)
    cell_style = ParagraphStyle("RouteCell", parent=body_style, fontSize=8, leading=10)

    story = [
        Paragraph("ORCA Route & Navigation Report", title_style),
        Paragraph(route_title, styles["Heading2"]),
        Spacer(1, 4 * mm),
    ]

    summary = [
        ["Distance", f"{total_km:.1f} km"],
        ["Nautical miles", f"{total_nm:.1f} NM"],
        ["Estimated time", str(transit)],
        ["Fuel savings", f"INR {cost_saved:,.0f}"],
        ["Risk level", str(risk_label)],
    ]
    summary_table = Table(summary, colWidths=[48 * mm, 52 * mm])
    summary_table.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#EAF4F7")),
        ("TEXTCOLOR", (0, 0), (0, -1), colors.HexColor("#0B3954")),
        ("FONTNAME", (0, 0), (0, -1), "Helvetica-Bold"),
        ("GRID", (0, 0), (-1, -1), 0.35, colors.HexColor("#B8D4DD")),
        ("PADDING", (0, 0), (-1, -1), 6),
    ]))
    story.extend([summary_table, Spacer(1, 7 * mm), Paragraph("Waypoints", styles["Heading2"])])

    waypoint_rows = [[
        Paragraph("Waypoint", cell_style), Paragraph("Coordinates", cell_style),
        Paragraph("Leg distance", cell_style), Paragraph("Bearing", cell_style),
        Paragraph("Advisory", cell_style),
    ]]
    for waypoint in waypoints:
        waypoint_rows.append([
            Paragraph(str(waypoint.get("name", "Waypoint")), cell_style),
            Paragraph(f"{float(waypoint.get('lat', 0.0)):.4f}°N, {float(waypoint.get('lon', 0.0)):.4f}°E", cell_style),
            Paragraph(
                "Start" if not waypoint.get("leg_distance_nm") else f"{float(waypoint['leg_distance_nm']):.1f} NM",
                cell_style,
            ),
            Paragraph(
                "Departure" if waypoint.get("leg_bearing") is None else f"{waypoint['leg_bearing']}°",
                cell_style,
            ),
            Paragraph(str(waypoint.get("notes", waypoint.get("advisory", ""))), cell_style),
        ])

    waypoint_table = Table(waypoint_rows, colWidths=[38 * mm, 32 * mm, 24 * mm, 24 * mm, 52 * mm], repeatRows=1)
    waypoint_table.setStyle(TableStyle([
    ...
    story.append(Paragraph("Generated by ORCA Marine Intelligence. Verify live weather and local maritime advisories before departure.", body_style))
...
""", unsafe_allow_html=True)
                    ...
                    if is_suspended:
                        ...
                        if waypoints:
                            pdf_data = _generate_route_pdf(
                                waypoints,
                                route_title=f"{start_lbl} to {end_lbl}",
                                total_km=total_km,
                                total_nm=total_nm,
                                transit=transit,
                            ...
                            clean_fn = f"orca_route_{str(start_lbl)[:8].strip()}_{str(end_lbl)[:8].strip()}.pdf".replace(" ", "_").replace(",", "")
                            col_gpx_act.download_button(
                                label=loc.get("btn_export_pdf", "📄 Export PDF"),
                                data=pdf_data,
                                file_name=clean_fn,
                                mime="application/pdf",
                                key=f"dl_pdf_{nav_key}",
                                use_container_width=True,
                                help="Download a PDF route report with waypoints and navigation details",
                            )
                            dest_wp = waypoints[-1]
